...
class Player
  ...
  def initialize(height, width, ground, data)
    @player_active = Gosu::Image.load_tiles('assets/Vrocket_thrust.png', 64, 64, { retro: true })
    @player_idle = Gosu::Image.load_tiles('assets/Vrocket_idle.png', 64, 64, { retro: true })
    @img_y = @vel_x = @vel_y = 0
    @angle = rand(0..90)
    @img_x = rand(0..width)
    @gravity = data['grav']
  ...
  def colliding
    @img_y > @height - @ground.ground_y(@img_x) - 18
  end
  ...
  def update
    ...
    if (Gosu.button_down? Gosu::KB_D) || (Gosu.button_down? Gosu::KB_RIGHT)
      @angle += TURNSPEED
    end
    @img_y += @vel_y
    @img_x += @vel_x
  ...
  def draw
    if @moving
      @player_active[@animation_state / 10].draw_rot(@img_x, @img_y, 0, @angle, 0.5, 0.5, @scale_x, @scale_y)
      @animation_state = (@animation_state + 1) % 30
    else
      @player_idle[0].draw_rot(@img_x, @img_y, 0, @angle, 0.5, 0.5, @scale_x, @scale_y)
    end
  end
end
